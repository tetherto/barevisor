...
module.exports = class Listener extends ReadyResource {
  constructor(vm, port, address, opts = {}) {
    ...
    // unset is bidirectional; stream drive moves bytes one way and says which
    this.writing = opts.writing
    this.pid = 0
    this.socket = null
  }
  ...
  async _open() {
    if (this.vm.dialsFirst) this.socket = await this._dial()
...
function script(listener) {
  ...
  const start = `socat -d -d ${direction(writing)}${vm.guestAddress(port)} ${address} >/dev/null 2>${LOG}${port} </dev/null &
pid=$!`
