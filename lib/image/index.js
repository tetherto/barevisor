...
module.exports = class Image extends ReadyResource {
  constructor(drive, opts = {}) {
    ...
    this.cache = opts.cache || path.join(storage.persistent(), 'barevisor')
    ...
    this.transport = opts.transport || 'vsock'
  ...
  async overlay() {
    ...
    if (this.transport === 'vport') {
      entries.push({ name: 'agent/vport', data: b4a.from(vport), mode: 0o100755 })
    }
