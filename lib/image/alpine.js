...
module.exports = class AlpineImage extends Image {
  ...
  listener() {
    if (this.transport === 'vport') {
      ...
      // for the agent rather than churning — and say so if it never arrives
      return `for i in $(seq 100); do [ -S /run/agent.sock ] && break; sleep 0.1; done
[ -S /run/agent.sock ] || fail agent

port=$(/agent/vport port-${this.agentPort}) || fail vport
echo "[linux] bridging $port"

while :; do
  socat -d -d GOPEN:$port UNIX-CONNECT:/run/agent.sock
  echo "[linux] bridge exited $?"
  sleep 0.2
done &`
    }
